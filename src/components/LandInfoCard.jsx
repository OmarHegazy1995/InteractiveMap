...
const LandInfoCard = ({ selectedLand, isInsideFilter, isBelowMap }) => {
  ...
  let baseClasses =
    "bg-gradient-to-r from-green-800 to-green-600 text-white p-4 rounded-xl shadow-lg border border-green-900";
  ...
  return (
    <div className={baseClasses}>
      ...
      <p className="mb-1">
        <span className="font-semibold">رقم المخطط:</span> {selectedLand.planNumber || "غير محدد"}
      </p>
      <p className="mb-1">
        <span className="font-semibold">الحالة:</span>{" "}
        {selectedLand.status === "مستثمر" || selectedLand.invested ? (
          <span className="text-red-400">مستثمرة</span>
        ) : selectedLand.status === "قيد الطرح" ? (
          <span className="text-yellow-300">قيد الطرح</span>
        ) : selectedLand.status === "تحت الإيجار" ? (
          <span className="text-orange-300">تحت الإيجار</span>
        ) : (
